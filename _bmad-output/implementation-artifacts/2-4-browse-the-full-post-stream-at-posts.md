...
| 2026-06-23 | Bug fix (Carrie, manual testing): restored the mobile hamburger drawer on `/posts/` by adding `partial "sidebar.html"` (disableSidebar, no placeholder) — our custom `main` layout had dropped it. Hidden on desktop, working on mobile (Playwright-verified). Same fix applied to `posts/single.html` (Story 2.2). |
