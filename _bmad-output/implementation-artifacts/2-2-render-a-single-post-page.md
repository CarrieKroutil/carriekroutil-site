...
| 2026-06-23 | Bug fix (Carrie, manual testing): mobile hamburger menu was dead on posts pages — our custom `main`-block layout dropped Hextra's `sidebar.html`, which renders the drawer the navbar hamburger toggles. Added `{{ partial "sidebar.html" (dict "context" . "disableSidebar" true "displayPlaceholder" false) }}` to `posts/single.html` (and `posts/list.html`); hidden on desktop, restored on mobile. Verified with Playwright at 390px: hamburger opens the Search/Posts/About/theme drawer. |
