...
When this page is filled in, it'll cover how to translate tech debt into the language of business impact — [slower delivery]({{< relref "/handbook/engineering-foundations/delivery-metrics-agile.md" >}}), more incidents, harder hiring — so it competes fairly on the roadmap instead of begging for scraps; how to tell prudent debt you took on purpose from the reckless kind that just accumulated; how to fold paydown into feature work so it isn't always a separate ask; and how to know when debt is cheap to carry versus when it's quietly strangling your velocity. Until then, the Go Deeper links carry the weight — Fowler's debt quadrant is the fastest way to make this a grown-up conversation.
